...
///Intermediate widget for `iced_native::widget::Text` with size `SIZE`
///
...
impl<const SIZE: u16> Text<SIZE> {
    pub fn new<T, R>(text: T) -> iced_native::widget::Text<R>
    where
        T: Into<String>,
        R: iced_native::text::Renderer,
    {
        iced_native::widget::Text::new(text).size(SIZE)
    }
